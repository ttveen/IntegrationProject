...
$$
which results in the estmated system $\hat{A}$, $\hat{B}$, $\hat{C}$ and $\hat{D}$. The residuals can be used to compute the covariance matrices \cite[p.~333]{FilteringIdentification}.
\begin{align*}
    ...
    \begin{bmatrix} \hat{Q} & \hat{A}\\ \hat{S}^T & \hat{R} \end{bmatrix} &= \lim_{N \xrightarrow{}\infty} \frac{1}{N} \begin{bmatrix} \hat{W}_{s,1,N} \\ \hat{V}_{s,1,N} \end{bmatrix} \begin{bmatrix} \hat{W}_{s,1,N}^T & \hat{V}_{s,1,N}^T \end{bmatrix}
\end{align*}